...
test("MCP tool catalog is deterministic and input schemas reject extra fields", () => {
  ...
  assert.equal(new Set(TOOL_DEFINITIONS.map((tool) => tool.name)).size, 11);
...
test("MCP server supports modern discovery and legacy initialization", () => {
  ...
  assert.equal(responses.find((response) => response.id === 2).result.tools.length, 11);
...
test("generic transitions cannot bypass dedicated worktree or delivery evidence tools", () => {
  const root = gitRepository();
